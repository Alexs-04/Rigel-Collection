...
export default function Suppliers() {
    ...
    const loadDetail = async (name) => {
        ...
        await loadDetailSafely({
            ...
            onSuccess: ({detail: resolvedDetail, products: resolvedProducts}) => {
            ...
            {isAdmin && (
                <section className="card" style={{padding: 20}}>
                    ...
                    <form onSubmit={addSupplier}
                        ...
                        <input className="input" placeholder="Correo" type="email" value={form.contactEmail}
                               onChange={(e) => onChangeForm('contactEmail', e.target.value)} required/>
                        <input className="input" placeholder="Telefono" value={form.phoneNumber}
                               onChange={(e) => onChangeForm('phoneNumber', e.target.value)} required/>
                        <input className="input" placeholder="Direccion" value={form.address}
                               onChange={(e) => onChangeForm('address', e.target.value)} required/>
                                ...
                                {detail && !loadingDetail && (
                                    <>
                                        <div style={{display: 'grid', gap: 10, marginBottom: 14}}>
                                            ...
                                            <input className="input" type="email" value={detail.contactEmail}
                                                   disabled={!editing} onChange={(e) => setDetail((prev) => ({
                                                ...prev,
                                                contactEmail: e.target.value
                                            }))}/>
                                            <input className="input" value={detail.phoneNumber} disabled={!editing}
                                                   onChange={(e) => setDetail((prev) => ({
                                                       ...prev,
                                                       phoneNumber: e.target.value
                                                   }))}/>
                                            <input className="input" value={detail.address || ''} disabled={!editing}
                                                   onChange={(e) => setDetail((prev) => ({
                                                       ...prev,
                                        ...
                                        {isAdmin && (
                                            <div style={{display: 'flex', gap: 8, marginBottom: 12}}>
                                                ...
                                                <button className="btn-ghost" type="button"
                                                        onClick={deleteSupplier}>Eliminar
                                                </button>
                                            </div>
                                        )}
                                        ...
                                        {products.length === 0 ? (
                                            <p className="text-muted" style={{marginTop: 0}}>Este proveedor no tiene
                                                productos asociados.</p>
                                        ) : (
                                            <div style={{display: 'grid', gap: 8}}>
